(function() {
  ...
  module.service('EmployeeService', ['$http', function($http) {
    ...
    this.next = function(page, pageSize) {
      ...
      console.log('GET /api/employees?page=' + page + '&size=' +
        pageSize);
      return $http.get('/api/employees?page=' + page + '&size=' +
          pageSize)
        .then(function(response) {
          return response.data;
        });
    ...
    this.findByFullName = function(fullName) {
      console.log(
        'GET /api/employees/search/findByFullName?fullName=' +
        fullName);
      return $http.get(
          '/api/employees/search/findByFullName?fullName=' + fullName
        )
        .then(function(response) {
          return response.data;
        });
  ...
  module.service('CustomerService', ['$http', function($http) {
      ...
      var customerList = $http.get('/api/customer')
        .then(function(response) {
          if (response.data._embedded) {
            console.log('getAll customer ' + response.data._embedded
              .customer.length);
    ...
    this.searchCustomer = function(searchField) {
      var params = {
        params: {
          customerName: searchField
        }
      };
      console.log('Searching customer by name ' + searchField);
      var customerList = $http.get(
          '/api/customer/search/findByCustomerName', params)
        .then(function(response) {
          if (response.data._embedded) {
            console.log('searchCustomer customer ' + response.data._embedded
              .customer.length);
            return response.data._embedded.customer;
          } else {
            return [];
        ...
        .then(function(response) {
          if (response.data._embedded) {
            ...
            customerList.forEach(function(entry) {
              ...
              entry.infoBox = '<h3>' + entry.customerName +
                '</h3><div class="infoWindowContent">' + entry.address +
                '</div>';
            });
            return customerList;
          } else {
  ...
  module.service('OfficeService', ['$http', function($http) {
    ...
    this.getAll = function() {
      var list = $http.get('/api/office')
        .then(function(response) {
          if (response.data._embedded) {
            console.log('getAll offices ' + response.data._embedded
              .office.length);
    ...
    this.getAllOfficesForMap = function() {
      return $http.get('/api/office')
        .then(function(response) {
          if (response.data._embedded) {
            console.log('getAll office ' + response.data._embedded.office
              .length);
            var list = response.data._embedded.office;
            list.forEach(function(entry) {
              entry.title = entry.office;
              entry.infoBox = '<h3>' + entry.office +
                '</h3><div class="infoWindowContent">' + entry.address +
                '<br>' + entry.telephone + '</div>';
            });
            return list;
          } else {
            return [];
